...
default_params = {
    ...
    'sample_length': 80000,
    'loss_smoothing': 0.99,
    'cuda': False,  # MLX doesn't use CUDA, so we set this to False
    'comet_key': None
}
...
def load_last_checkpoint(checkpoints_path):
    ...
    if len(checkpoint_paths) > 0:
        ...
        return (mx.load(checkpoint_path), epoch, iteration)
    else:
        return None
...
def main(exp, frame_sizes, dataset, **params):
    ...
    optimizer = gradient_clipping(optim.Adam(learning_rate=0.001))
    ...
    if checkpoint_data is not None:
        ...
        predictor.load_weights(state_dict)
