...
from mlx.data import Dataset, DataLoader as DataLoaderBase
...
class FolderDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return mx.concatenate([
            mx.zeros((self.overlap_len,), dtype=mx.int32) + utils.q_zero(self.q_levels),
            utils.linear_quantize(
                mx.array(seq), self.q_levels
            )
        ])
...
class DataLoader(DataLoaderBase):
    ...
    def __iter__(self):
        for batch in super().__iter__():
            batch_size, n_samples = batch.shape
